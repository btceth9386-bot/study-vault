...
3. 絕對不能把建議直接套用到正式概念檔。所有概念修正、合併、拆分、補充都只能寫在 refine report 中，交由後續獨立證據驗證與 promotion 流程處理；使用者只處理證據無法解決的重大選擇。
4. 若發現 `concepts/` 需要更新，只能在報告中提出明確建議，包含目標檔案、原因、建議修改內容摘要。
5. 若資料不足以做出判斷，請在報告中標記「需要補充證據」；只有存在多個合理且 materially different 的方向時才標記 `needs-decision` 並提出一個使用者問題。
...
- 本週最應優先交由獨立 reviewer 驗證的 3-5 個項目。
- 每個項目說明原因、涉及檔案、證據缺口與建議後續流程。
- 僅對 `needs-decision` 項目列出一個使用者問題。
