...
- **Tools**: Kiro CLI (openab's ACP agent backend), Langfuse (in-cluster `langfuse` namespace), a distinct judge model provider (see Prerequisites)
...
**Why the task model and the judge model must not be the same provider or model family.** This lab deliberately uses two separate model providers: Kiro CLI executes the task under test, and a distinct provider — never dispatched through Kiro CLI — grades its output. Two independent reasons force this:

1. *Cost attribution.* The `Stop` hook fires on every Kiro CLI response. If the judge also called Kiro CLI, each dataset item would produce two hook-triggered usage-log entries (one for the task, one for the judge), and `credit_usage_run_evaluator()` would report the combined cost of the product *and* the evaluation tooling as if it were the product's cost alone. Evaluation cost is not product cost and must not be attributed to the variant under test.
2. *Self-preference bias.* Using a model to score its own family's output is a known source of systematic bias — models tend to rate outputs from their own family more favorably, independent of actual quality. Established practice is that the generator and the judge must come from different model families, not just different individual calls.
...
- **A distinct judge provider**: authenticated separately from Kiro CLI, via its own credential (`JUDGE_API_KEY`), and belonging to a **different model family than whatever Kiro CLI executes the task with**. This scaffold does not pin you to one vendor — `scripts/lib/judge_client.py` dispatches on `JUDGE_PROVIDER` (`anthropic`, `openai`, or `gemini` out of the box; add another `_call_<provider>` function there if you need one not listed). Use whichever provider you actually hold credentials for. The non-overlap requirement is **not checked automatically**: Kiro CLI's documented model roster ([kiro.dev/docs/models](https://kiro.dev/docs/models/)) spans OpenAI, Anthropic, DeepSeek, MiniMax, GLM, and Qwen, so if you pick, say, `anthropic` as the judge, you must also confirm your Kiro CLI task-side `--model` isn't a Claude variant — verify this yourself before trusting any run's results.
...
- **Cost risk, now split across two billing sources**: each dataset run makes ~8 Kiro CLI calls (task only — the judge no longer touches Kiro CLI). Two variants → ~16 Kiro CLI calls for the dataset runs, plus the model-compare task's ≤9 (3 items × up to 3 models) → **~25 Kiro CLI calls total**, billed in Kiro credits. The judge now runs as a fully separate cost line: ~8 judge calls per variant + ≤9 for model-compare → **~25 judge-provider calls total**, billed against whichever `JUDGE_PROVIDER` you configured — check that provider's current pricing before running, since this lab did not previously have a second billing source. Check `/usage` before and after on Kiro if you're on a metered plan; check your judge provider's usage dashboard separately.
...
- `scripts/lib/kiro_client.py` — subprocess wrapper around `kiro-cli chat --no-interactive`, ANSI stripping, latency timing. Used for **task** calls only.
- `scripts/lib/judge_client.py` — dispatches to whichever provider `JUDGE_PROVIDER` names (`anthropic` / `openai` / `gemini` supported out of the box). Used for **judge** calls only — never imported by anything that also calls `kiro_client`.
...
- `scripts/log_usage.sh` — the script the `Stop` hook is supposed to invoke; appends one JSON line per **Kiro CLI** response to `usage-log.jsonl`. Never fires for judge calls, because judge calls never go through Kiro CLI.
...
- `scripts/model_compare.py` — the medium-complexity task: runs a 3-item subset through 2–3 Kiro CLI models with the *same* (improved) prompt, scores each via the judge provider, writes `runs/model-compare.json`.
...
- `scripts/setup.sh` — prerequisite + smoke-test script (kiro-cli present, Langfuse reachable, judge provider credential present, one live `/model` + prompt round trip, one usage-log line).

## Your core task

Implement `judge_score()` in `scripts/lib/langfuse_glue.py` (currently `raise NotImplementedError`). It is an item-level Langfuse evaluator: given `(input, output, expected_output, metadata)` for one dataset item, call the **judge provider** — via `judge_client.call_judge()`, never `call_kiro()` — with a rubric prompt, and return an `Evaluation(name="judge_correctness", value=<1-5 or -1>, comment=<reason>)`.

The five numbered requirements are spelled out as a docstring directly above the `raise NotImplementedError` — including how to handle `item-07`'s `AMBIGUOUS_BY_DESIGN:` expected output, which is the seam that connects your rubric design to Failure case 1 below, and including the requirement that the judge call must go through `judge_client`, not `kiro_client`.

## Steps

1. Read the scaffold (`scripts/lib/langfuse_glue.py`, `scripts/lib/kiro_client.py`, and `scripts/lib/judge_client.py` first).
...
5. Run `python scripts/run_variant.py --variant baseline`, then rename/clear `usage-log.jsonl`, then run `python scripts/run_variant.py --variant improved`. Watch for Failure case 2 between these two runs (see below — do not "fix" it before it happens once). A healthy run's `usage-log.jsonl` should have exactly 8 new lines (task calls only — judge calls never appear here).
...
**Case 2 — the credit-usage hook silently doesn't fire.** `.kiro/hooks/log-usage-on-stop.json` uses `"trigger": "PostStop"`. The real Kiro trigger name (per [kiro.dev/docs/hooks](https://kiro.dev/docs/hooks/#available-triggers)) is `"Stop"` — `PostStop` matches nothing, so the hook is installed but inert. Symptom: after running the `improved` variant, `usage-log.jsonl` has zero new lines (not the expected 8), `credit_usage_run_evaluator()` reports `credits_logged: 0` for that run in Langfuse, and `scripts/compare_report.py` prints an explicit `FAIL` line pointing at this file. This is deliberately the same class of failure real teams hit: a cost-tracking integration that looks configured but isn't wired to the trigger the tool actually emits, and nothing loud tells you until you go looking for the data that should be there and isn't.
...
- For each healthy dataset run, `credit_usage_run_evaluator()`'s `credits_logged` value equals exactly **8** — task calls only. If you see 16, the judge is still (incorrectly) calling `kiro_client` instead of `judge_client`; if you see 0, that's Failure case 2, not this bug.
- You can explain, in one sentence each: (a) why item-07's score does not falsify your judge, (b) what in the hook config caused Failure case 2 and how you found it (not just "I fixed it"), and (c) why the judge call must not go through Kiro CLI even though it would have been simpler to reuse `call_kiro()`.
